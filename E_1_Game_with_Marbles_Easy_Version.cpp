...
void solve() {
    ...
    ll n, turn = 0, sum_a = 0, sum_b = 0;
    cin >> n;
    vector<ll> a(n), b(n);
    vector<pair<ll, ll>> benefit;
    for (ll i = 0; i < n; i++)
    {
        cin >> a[i];
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        if (turn % 2 == 0)
        {
            a[benefit[i].second] -= 1;
            b[benefit[i].second] = 0;
        }
        else
        {
            b[benefit[i].second] -= 1;
            a[benefit[i].second] = 0;
        }
        turn++;
    }
    for (ll i = 0; i < n; i++)
