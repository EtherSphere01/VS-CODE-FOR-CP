...
ll check(ll i, ll j, string &a, string &b, string &c)
{
    if (a.size() == i and b.size() == j)
    {
        return 0;
    }
    if (dp[i][j] != -1)
    ...
    if (i < a.size())
    {
        ll temp = check(i + 1LL, j, a, b, c) + (a[i] != c[pos]);
        ans = min(ans, temp);
    }
    if (j < b.size())
    {
        ll temp = check(i, j + 1LL, a, b, c) + (b[j] != c[pos]);
        ans = min(ans, temp);
    }
    return dp[i][j] = ans;
}
