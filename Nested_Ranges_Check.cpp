...
void solve()
{
    ...
    for (int i = 0; i < n; i++){
        ...
        if(max_end_so_far != INT_MIN && max_end_so_far >= current.end){
            is_contained_by_other[current.original_index] = 1;
        }
        max_end_so_far = max(max_end_so_far, current.end);
    }

    int min_end_so_far = INT_MAX;
    for (int i = n - 1; i >= 0; i--){
        const auto& current = intervals[i];

        if(min_end_so_far != INT_MAX && min_end_so_far <= current.end){
            contains_other[current.original_index] = 1;
        }
        min_end_so_far = min(min_end_so_far, current.end);
    }

    for (int i = 0; i < n; i++){
        cout << contains_other[i] << sp;
    }
    cout << nline;
    for (int i = 0; i < n; i++){
        cout << is_contained_by_other[i] << sp;
    }
    cout << nline;
}
