...
void solve()
{
    ...
    // Prefix and suffix maxima over gaps
    vector<long long> pref(m), suf(m);
    pref[0] = d[0];
    for (int i = 1; i < m; ++i)
    ...
    // Remove exactly one stop: index k in [1, n-2]
    for (int k = 1; k <= n - 2; ++k)
    {
        long long other = 0;
        if (k - 2 >= 0)
        ...
        if (k + 1 <= m - 1)
            other = max(other, suf[k + 1]);
        long long merged = d[k - 1] + d[k];
        long long cand = max(other, merged);
        ans = min(ans, cand);
    }
