...
void solve()
{
    ...
    for (ll i = 1; i <= n; i++)
    {
        if (a[i] == 1)
        {
            pre_sum1[i] = pre_sum1[i - 1] + 1;
            pre_sum0[i] = pre_sum0[i - 1];
        }
        else if (a[i] == 0)
        {
            pre_sum1[i] = pre_sum1[i - 1];
            pre_sum0[i] = pre_sum0[i - 1] + 1;
        }
    ...
    while (q--)
    {
        ll l, r;
        cin >> l >> r;
        ll ones = pre_sum1[r] - pre_sum1[l - 1];
        ll zeros = pre_sum0[r] - pre_sum0[l - 1];
        if (ones % 3 || zeros % 3)
        {
            out(-1);
            continue;
